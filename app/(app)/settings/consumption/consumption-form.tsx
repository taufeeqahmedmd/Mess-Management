  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4">
      ...
      <p className="rounded-sm border border-line bg-surface-2 px-3 py-2.5 text-[11.5px] leading-relaxed text-muted-2">
        Pick one or both models. When both are enabled, taps resolve <strong className="font-semibold text-muted">coupon first, then
        wallet</strong>.
      </p>
      ...
      <div className="rounded-md border border-line bg-surface shadow-sm">
        <div className="hidden grid-cols-[1.3fr_2fr_1.3fr_1fr] gap-4 border-b border-line bg-surface-2 px-5 py-2.5 text-[10.5px] font-bold uppercase tracking-[0.07em] text-muted-2 sm:grid">
          <span>Category</span>
          <span>Models</span>
        ...
        {rows.map((r) => {
          ...
          return (
            <div
              key={r.id}
              className="flex flex-col gap-3 border-b border-line p-5 last:border-0 sm:grid sm:grid-cols-[1.3fr_2fr_1.3fr_1fr] sm:items-center sm:gap-4 sm:px-5 sm:py-3.5"
            >
              <div className="font-medium text-ink">{r.name}</div>

              <div>
                <span className="mb-1.5 block text-[11px] font-semibold uppercase tracking-[0.05em] text-muted-2 sm:hidden">Models</span>
                <MultiSelect
